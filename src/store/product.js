...
export const useProductStore = create((set, get) => ({
    ...
    fetchProducts: async() => {
        const res = await fetch('https://productapi-back.onrender.com/create/api/products');
        const data = await res.json();
        set({ products: data.data })
    },
    deleteProduct: async (id) => {
        const res = await fetch(`https://productapi-back.onrender.com/create/api/product/${id}`, {
            method: "DELETE",
        });
        const data = await res.json();
